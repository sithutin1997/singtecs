...
const ServiceDetail = ({header, rightBody = []}) => {
  ...
  return (
    <div className='flex flex-col flex-y-10 p-12'>
      <h1 className={header ? `font-gotham-book text-5xl` : 'hidden'}>{ header ? header : `Diet`}</h1>
        ...
        <div className='w-2/3'>
          {
            rightBody.map((right,index) => {
              if(right.body.length > 5) {
                return (
                  <div key={index}>
                      <h2 className='font-gotham-bold text-cyan-500 text-3xl'>{right.listNumber}</h2>
                      <h3 className='font-gotham-bold text-lg'>{right.header}</h3>
                      <ul className='list-disc'>
                        {
                          right.body.map((element,index) => {
                          return <li key={index} className='font-gotham-thin font-sm'>{element}</li>
                        })
                        }
                      </ul>
                    </div>
              ...
              return (
                ...
                <div key={index}>
                  ...
                  {
                    right.body.map((element,index) => {
                     return <p key={index} className='font-gotham-thin font-sm'>{element}</p>
                    })
                  }
                </div>
              )
